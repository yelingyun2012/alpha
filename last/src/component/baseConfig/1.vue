                    ...
                    Tabs(type="card")
                        TabPane(label="归属站点")
                            RadioGroup(v-model="vertical", vertical)
                                template(v-for='list of testItem')
                                    Radio(label="adasd")
                        ...
                        TabPane(label="基本属性")
                            ...
                            p.BaseAttribute(style="font-size: 14px;color: #323232;padding:10px")
                                ...
                                span.BaseAttribute(style="font-size: 14px;color: #323232;padding:10px")
                            ...
                            p.BaseAttribute2(style="font-size: 14px;color: #323232;width:50%;height:120px;padding-left: 10px;")
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkIn () {
        if ($('.action-group div').length !== 0) {
          this.handleTraverseTree()
          ...
          let pageModelPropertyResult = [
            {
              propertyName: 1,//页面模型名
              urlPropertyExtract: 1,
              parentId: 1,
              modelId: 1,
              groupType: 1,//http://192.168.10.151:8181/pagemodel/add
              parseType: 1,
              matchExpression: 1,
            }
          ]
...
//          let quit = this.$route.query.plan
//          let pageModelResult = {
//            modelId: quit,
//            modelName: this.pagename,//页面模型名称
//            contentType: this.modelvalue,//内容提供方式
...
////          ])
//
//          let pageModelPropertyResult = JSON.stringify([])
...
//      let quit = this.$route.query.plan//---------------------------------- taskid
//      console.log(quit)
//
...
//      axios.post('pagemodel/query', params).then(response => {
//        let data = response.data.data
//        that.pagename = data.modelName
//        that.vertical = data.modelType
//        that.modelvalue = that.modelType[data.browserType - 1].value  //模型类型
//        that.Contentvalue = that.Contentmode[data.browserType - 1].value //内容提供方式
//        that.regular = data.modelRegularExpression //模型政策表达式
//        that.Allowscraw = data.createTime
//        that.NotAllowscraw = data.createTime
//        that.extractvalue = that.Listrule[data.urlExtractable].value
//        that.urlattributes = data.createTime
//        console.log(response)
//        console.log('详情')
//      })
//
