  ...
  section.user-panel
    Form(:label-width="100", ref="formValidate", :model="formValidate", :rules="ruleValidate")
      FormItem(label="用户名：", prop="userName")
        Input(placeholder="请输入用户名", v-model="formValidate.userName", :maxlength="250")
      FormItem(:label="$route.params.operate==='add'?'密码：':'新密码：'", prop="password")
        Input(placeholder="请输入密码", v-model="formValidate.password", :maxlength="50")
      FormItem(label="确认新密码：", v-if="$route.params.operate==='alter'", prop="newPassword")
        Input(placeholder="请输入新密码", v-model="formValidate.newPassword", :maxlength="50")
      FormItem(label="联系电话：")
        Input(placeholder="请输入联系电话", v-model="userPhone", :maxlength="13")
      FormItem(label="Email：")
        Input(placeholder="请输入Email地址", v-model="email", :maxlength="50")
