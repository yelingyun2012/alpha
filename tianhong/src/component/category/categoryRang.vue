                ...
                .table-list
                    table.table
                        ...
                        template(v-for='(item,index) in tabData')
                            tr
                                td(rowspan="3").ivu-table-column-center {{index+1}}
                        ...
                        <!--tr-->
                        <!--td(colspan="10",style='text-align: center;padding: 15px;',v-if='tabData=""') 暂无数据-->
</template>
<script>
  ...
  import { rangCategory, PriceZone } from '../../api/actions'
...
//        }
        ...
        rangCategory(data).then(response => {
          _this.treeData = []
...
//        })
      ...
      derive () {
        let deriveUrl = `http://121.9.245.185:1188/Ajax/Export.ashx?PlatId=${this.activeIndex + 2}&Level=${this.goodsLevel}&CategoryId=${this.goodsId}&FileName=品类价格带导出数据 ${moment(new Date).format('YYYY-MM-DD')}&method=CategoryPriceZone&Token=${getToken('Token')}`
